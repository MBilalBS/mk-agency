...
extend({ MeshLineGeometry, MeshLineMaterial })
 ...
 function Badge() {
  ...
  return (
    <div className='BadgePos'>
  <div className="badge-canvas">
    <Canvas camera={{ position: [0, 0, 13], fov: 30 }}
      style={{ background: 'transparent' }}>
      <Physics interpolate gravity={[0, -40, 0]} timeStep={1 / 60}>
        <Band isRight={isRight}/>
      </Physics>
    </Canvas>
  </div>

  <div className="toggle-section">
    <span className={`founder-name ${!isRight ? 'active' : ''}`}>Mohamed Bounouar</span>
    <div className="toggle" onClick={() => setIsRight(!isRight)}>
      <div className={`toggle-thumb ${isRight ? 'right' : ''}`}></div>
    </div>
    <span className={`founder-name ${isRight ? 'active' : ''}`}>Khalil Keroui</span>
  </div>
</div>
  )
}

function Band({ isRight }) {
...
const targetY = isRight ? Math.PI : 0
const currentY = ((rot.y % (2 * Math.PI)) + 2 * Math.PI) % (2 * Math.PI)
const diff = targetY - currentY
if (Math.abs(diff) > 0.01) {
  card.current.setAngvel({ x: ang.x, y: diff * 3, z: ang.z })
} else {
